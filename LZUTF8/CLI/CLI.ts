namespace LZUTF8
{
	export class CLI
	{
		static start()
		{
			...
			{
				log("No destination file specified");
				process.exit(1);
			}
			...
			else if (command == "t")
			{
				...
				{
					let sourceFileContent = ArrayTools.bufferToUint8Array(NodeFS.readFileSync(sourceFilePath));
					let temporaryFileContent = ArrayTools.bufferToUint8Array(NodeFS.readFileSync(temporaryFilePath));
